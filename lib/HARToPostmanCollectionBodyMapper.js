 ...
 * @param {text} mimeType The mime type of the entry response
 * @returns {object} Postman's body structure
 */
function handleUnsupportedResponseType(mimeType) {
  return {
    body: `${mimeType} content type is not supported in the response body`,
...
function mapBody(harRequest, options) {
  let handledBody;
  if (harRequest.bodySize > 0 || (harRequest.postData && harRequest.postData.text)) {
  ...
  if (harResponse.content.size > 0) {
    ...
    switch (mimeType) {
      ...
      case SUPPORTED_BODY_TYPES.applicationXml:
        ...
        break;
      default:
        handledBody = handleUnsupportedResponseType(mimeType);
        break;
    }
  }
  return handledBody;
