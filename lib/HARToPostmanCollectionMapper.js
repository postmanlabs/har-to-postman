...
/**
 * Generates a cookie header from all the HAR request cookies
 * @param {Array} harRequestCookies array of cookies
 * @param {object} options the process option
 * @returns {object} postman's header
 */
function filterCookiesFromHeader(harRequestHeaders, options) {
  if (!options) {
    return harRequestHeaders.filter((header) => { return header.name !== 'Cookie'; });
  }
  let {
    includeCookies
  } = options;
