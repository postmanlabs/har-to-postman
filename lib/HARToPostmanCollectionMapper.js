...
function excludeHeaderFromList(headersList, headerName) {
  return headersList.filter((header) => { return header.name !== headerName; });
}
...
function excludeCookieHeader(headersList) {
  return excludeHeaderFromList(headersList, 'Cookie');
}
 ...
 * @returns {Response} postman response
 */
function processOptionatedResponse(harResponse, item, options) {
...
function generateItem(harContentEntry, urlVariablesData, options) {
  ...
  item.response = processOptionatedResponse(harResponse, item, options);
  return item;
}
