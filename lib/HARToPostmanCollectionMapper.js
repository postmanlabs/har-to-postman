 ...
 */
function exlcudePseudoHeader(header) {
...
function getItemRequestHeaders(harRequestHeaders, options) {
  let filteredHeaders = filterCookiesFromHeader(harRequestHeaders, options).filter(exlcudePseudoHeader),
...
function getItemResponseHeaders(harResponseHeaders) {
  return harResponseHeaders.filter(exlcudePseudoHeader).map((header) => {
    return { key: header.name, value: header.value };
  });
}
